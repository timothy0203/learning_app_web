...
function loadEnv() {
  try {
    ...
    // .env.local 不存在（如 GitHub Actions），跳過
  }
}
...
請將每一節的經文「逐詞拆解」，為每個關鍵詞創造精準諧音和具體畫面。
...
【逐詞拆解規則 — 非常重要】
- 經文中的「每一個字/詞」都要拆解，不能跳過任何字
- 每個字/詞都要有精準諧音（聲母韻母必須對上）
- 每個字/詞都要有具體的視覺畫面描述（20-40字）
- 前後詞之間要有空間或邏輯關聯，像連鎖一樣一個接一個
- 重複出現的概念（神、耶穌、以色列、聖靈等）請用固定的具象化形象
- 故事場景要豐富多變，避免相鄰章節用類似背景

【輸出格式】
{"segments":[{"verse":1,"original":"原詞","pun":"精準諧音","image":"具體畫面描述（20-40字）"},{"verse":1,"original":"原詞2","pun":"精準諧音2","image":"具體畫面描述"},...],"story":"將所有畫面串聯成一段流暢的完整故事"}
